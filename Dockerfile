...
RUN mv /etc/apt/sources.list /etc/apt/sources.list.backup \
        ...
        -e 's#archive.ubuntu.com#de.archive.ubuntu.com#g' /etc/apt/sources.list.backup > /etc/apt/sources.list
...
RUN cd /opt && git clone --depth=1 https://code.videolan.org/videolan/x264.git && cd x264 \
...
RUN cd /opt && wget -nv -c "https://phoenixnap.dl.sourceforge.net/project/exiftool/Image-ExifTool-12.25.tar.gz" -O "Image-ExifTool-12.25.tar.gz" \
        && gzip -dc Image-ExifTool-12.25.tar.gz | tar -xf - && cd Image-ExifTool-12.25  \
        && perl Makefile.PL && make install \
        && cd ../ && rm -rf Image-ExifTool-12.25
